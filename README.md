...
`splitter.py` is the python script that was used to parse and clean up the original 70gb evaluation file from the [lichess.org open database](https://database.lichess.org/#evals).
...
The new model weights and biases will be automatically saved to the `model_weights.pt` file in `/Training/models` (not `/models`), so if you wish to use the trained model make sure to replace the old one in `/models` with the trained one.
